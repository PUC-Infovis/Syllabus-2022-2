...
// Es el mismo Ejemplo 1, pero cambiamos el dominio para usar potencias de 2.
...
const escalaLinealBaseDos = d3
  .scaleLinear()
  .domain([1, 2 ** 16])   // Dominio potencia de 2
  .range([0, 800]);       // Rango de 0 a 800 pixeles
const ejeLinealBaseDos = d3.axisBottom(escalaLinealBaseDos);
svgEj2.append("g").attr("transform", `translate(100, 0)`).call(ejeLinealBaseDos);
...
svgEj2.append("g").attr("transform", `translate(100, 75)`).call(ejeLogaritmicoBaseDos);


// Eje raíz cuadrada en mismo dominio y rango
const escalaRaizCuadradaBaseDos = d3
  .scalePow()
  .exponent(0.5)
  .domain([1, 2 ** 16])   // Dominio potencia de 2
  .range([0, 800]);       // Rango de 0 a 800 pixeles
const ejeRaizCuadradaBaseDos = d3.axisBottom(escalaRaizCuadradaBaseDos);
svgEj2.append("g").attr("transform", `translate(100, 150)`).call(ejeRaizCuadradaBaseDos);
